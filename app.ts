...
function reverseInt(num: number) {
  ...
  const reversedStr = numToString.split("").reverse().join("");
...
function maxChar(str: string) {
  ...
  let charCountObj: {[index: string]: number} = {};
  let maxChar = "";
  let maxCount = 0;
  ...
  return { charCountObj, maxChar, maxCount };
}
...
function anagram(str1: string, str2: string) {
  ...
  const str1CountObj: {[index: string]: number} = {};
...
function caesarCypher(str: string, shift: number) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    const newIdx = (idx + shift) % alpha.length;
...
function findIndexOf(arr: string[], item: string) {
  for (let i = 0; i < arr.length; i++) {
    const arrItem = arr[i];
    if (arrItem === item) {
      return i;
...
function stringPatternSearch(text: string, pattern: string) {
  let count = 0;
...
function bubbleSorting(arr: number[]) {
  for (let i = 0; i < arr.length; i++) {
